...
' Clasa pentru trimiterea informatiilor catre server in format XML
' intocmai ca un form HTML
Class clsXMLBubble
	Private XMLDom
	Private objStatus
	...
	Public Sub LoadFromForm(objForm)
		Dim FrmElem
		
		For Each FrmElem In objForm
			If FrmElem.type = "file" Then
				AddData FrmElem.id, FileToBytearray(FrmElem.value)
			Else
				AddData FrmElem.id, FrmElem.value
			End If
		Next
	End Sub
